...
### Ejercicio N°2:
Se soluciona montando volumenes con los respectivos archivos de config.yaml y config.ini en el cliente y en el servidor.
...
El servidor utiliza un funcion __graceful_shutdown(), que es invocada cuando se corta la ejecucion. La misma se encarga de cerrar todos los sockets abiertos y generar los correspondientes logs. Ademas, se agrega un timeout al socket del servidor, para que no se quede escuchando el socket cuando llega la señal.
...
La comunicación puede ser 
    ...
    - Se encodean los datos de la forma "agency|name|surname|document|birthDate|number\n"
...
Se implementa una politica de reintentos con un maximo establecido. La misma en los siguientes ejercicios es eliminada ya que considero que podemos confiar en que los mecanismos de envio son suficientes para garantizar que lleguen en los que respecta a este trabajo practico. Seria una gran mejor a trabajar.
...
El protocolo modificado será:
    ...
    - El cliente un batch de n apuestas, divididas por "\n". Se mantiene el header con el largo del payload
...
Por ultimo, en el case de que se reciba una SIGNAL que finaliza la ejecucion, se cerraran todos los sockets de clientes que el servidor mantiene en la variable client_sockets. Ademas haciendo en ultima instancia, un join de todos los threads.
